	...
	class FOUR7_Framework_Bootstrap extends FOUR7_Framework_Core {
		...
		public function __construct() {
			...
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				...
				if ( $fs_settings['navbar_social'] == 1 ) {
					if ( $fs_settings['navbar_social_style'] == 1 ) {
						add_action( 'four7_inside_nav_end', array( $this, 'navbar_social_bar' ) );
					} else {
						add_action( 'four7_inside_nav_end', array( $this, 'navbar_social_links' ) );
					}
				}

				if ( isset( $fs_settings['retina_toggle'] ) && $fs_settings['retina_toggle'] ) {
		...
		function navlist_widget_init() {
			...
			register_widget( 'Four7_Nav_Menu_Widget' );
		}
		 ...
		 * Use Four7_Nav_Walker() by default
		 */
		function nav_menu_args( $args = '' ) {
			...
			if ( ! $args['walker'] ) {
				$nav_menu_args['walker'] = new Four7_Nav_Walker();
			}

			if ( ! $args['fallback_cb'] ) {
				$nav_menu_args['fallback_cb'] = 'Four7_Nav_Walker::fallback';
			}
		...
		function breadcrumbs() {
			global $fs_settings, $fs_breadcrumbs;

			if ( is_front_page() || ( isset( $fs_settings['breadcrumbs'] ) && $fs_settings['breadcrumbs'] == 0 ) ) {
				return;
			}

			if ( isset( $fs_settings['site_style'] ) && $fs_settings['site_style'] != 'fluid' ) {
				$class = 'container';
			} else {
				$class = 'fluid';
			}

			if ( class_exists( 'Four7_Breadcrumbs' ) ) {
				echo '<div class="breadTrail ' . $class . '">';
				echo $fs_breadcrumbs->breadcrumb( false );
				echo '</div>';
			}
		}
		...
		public function make_dropdown_button( $color = 'primary', $size = 'medium', $type = null, $extra = null, $label = '', $content = '' ) {
			...
			$return .= '<button type="button" class="' . $fs_framework->button_classes( $color, $size, $type, 'dropdown-toggle' ) . '" data-toggle="dropdown">';
		...
		public function styles() {
			...
			if ( Four7_Color::get_brightness( $body_bg ) > 80 ) {
			...
			// Calculate grays
			if ( isset( $fs_settings['body_bg'] ) && ! empty( $fs_settings['body_bg'] ) ) {
			 ...
			 * LAYOUT
			 */
			if ( isset( $fs_settings['screen_tablet'] ) && ! empty( $fs_settings['screen_tablet'] ) ) {
				$screen_sm = filter_var( $fs_settings['screen_tablet'], FILTER_SANITIZE_NUMBER_INT );
			}

			if ( isset( $fs_settings['screen_desktop'] ) && ! empty( $fs_settings['screen_desktop'] ) ) {
				$screen_md = filter_var( $fs_settings['screen_desktop'], FILTER_SANITIZE_NUMBER_INT );
			}

			if ( isset( $fs_settings['screen_large_desktop'] ) && ! empty( $fs_settings['screen_large_desktop'] ) ) {
				$screen_lg = filter_var( $fs_settings['screen_large_desktop'], FILTER_SANITIZE_NUMBER_INT );
			}
			 ...
			 * BRANDING
			 */
			$brand_primary = '#' . str_replace( '#', '', Four7_Color::sanitize_hex( $fs_settings['color_brand_primary'] ) );
			$brand_success = '#' . str_replace( '#', '', Four7_Color::sanitize_hex( $fs_settings['color_brand_success'] ) );
			$brand_warning = '#' . str_replace( '#', '', Four7_Color::sanitize_hex( $fs_settings['color_brand_warning'] ) );
			$brand_danger  = '#' . str_replace( '#', '', Four7_Color::sanitize_hex( $fs_settings['color_brand_danger'] ) );
			$brand_info    = '#' . str_replace( '#', '', Four7_Color::sanitize_hex( $fs_settings['color_brand_info'] ) );
			...
			if ( $fs_settings['font_jumbotron_heading_custom'] == 1 ) {
				...
				$jumbotron_headers_text_color  = '#' . str_replace( '#', '', Four7_Color::sanitize_hex( $font_jumbotron_headers['color'] ) );
			...
			if ( isset( $fs_settings['navbar_margin_top'] ) && ! empty( $fs_settings['navbar_margin_top'] ) ) {
				$variables .= '@navbar-margin-top:       ' . $fs_settings['navbar_margin_top'] . 'px;';
			} else {
		...
		public function compiler() {
			...
			if ( isset( $fs_settings['minimize_css'] ) && $fs_settings['minimize_css'] == 1 ) {
				$compress = true;
			} else {
				$compress = false;
			}
			...
			try {
				...
				if ( isset( $fs_settings['user_less'] ) && ! empty( $fs_settings['user_less'] ) ) {
					$parser->parse( $fs_settings['user_less'] );
				}

				// Get the extra variables & imports
				$extra_vars = do_action( 'fs_bootstrap_less_vars' );
				$parser->parse( $extra_vars );

				// Add a filter to the compiler
				$parser->parse( apply_filters( 'four7_compiler', '' ) );
			...
			return apply_filters( 'four7_compiler_output', $css );
		}
		...
		public function include_wrapper() {
			...
			return $fs_layout->include_wrapper();
		}
	}
}