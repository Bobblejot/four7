	...
	class Four7_Advanced {
		...
		function __construct() {
			...
			if ( isset( $fs_settings['post_excerpt_length'] ) ) {
				define( 'POST_EXCERPT_LENGTH', $fs_settings['post_excerpt_length'] );
			}
		...
		*/
		...
		function options( $sections ) {
			...
			$section = apply_filters( 'four7_module_advanced_options_modifier', $section );
		...
		function user_css() {
			...
			if ( trim( $header_scripts ) != '' ) {
				wp_add_inline_style( 'four7_css', $header_scripts );
			}
		}
		...
		function scripts() {
			...
			if ( $settings['retina_toggle'] == 1 ) {
				wp_register_script( 'retinajs', four7_JS . '/vendor/retina.js', false, null, true );
