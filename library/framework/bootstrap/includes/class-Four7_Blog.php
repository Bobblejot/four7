...
if ( ! class_exists( 'Four7_Blog' ) ) {
    ...
    class Four7_Blog {
        ...
        function __construct() {
            ...
            if ( $fs_settings['single_meta_tags'] == 0 ) {
                add_filter( 'four7_the_tags', 'four7_blank' );
            }
            if ( $fs_settings['single_meta_cats'] == 0 ) {
        ...
        function options( $sections ) {
            ...
            $fields[] = array(
                'title'     => __( 'Show Post Meta Tags in single posts', 'four7' ),
                'desc'      => __( 'Toggle Post Meta Tags showing in the footer of single posts. Default: ON.', 'four7' ),
                'id'        => 'single_meta_tags',
                'default'   => 1,
                'type'      => 'switch',
            );
            ...
            $fields[] = array( 
				'title'     => __( 'Post Meta Tags Panel Styling', 'four7' ),
				'desc'      => __( 'Select between standard Bootstrap\'s panel classes', 'four7' ),
				'id'        => 'single_meta_tags_panel_class',
				'default'   => 'default',
				'type'      => 'select',
				'required'    => array('single_meta_tags','=',array('1')),
				'options'   => array(
				    'none'    => 'none',
					'default'    => 'Default',
					'primary'    => 'Primary',
            ...
            $fields[] = array(
                'title'     => __( 'Show Post Meta Categories in single posts', 'four7' ),
                'desc'      => __( 'Toggle Post Meta Categories showing in the footer of single posts. Default: ON.', 'four7' ),
                'id'        => 'single_meta_cats',
                'default'   => 1,
                'type'      => 'switch',
            );
            ...
            $fields[] = array( 
				'title'     => __( 'Post Meta Categories Panel Styling', 'four7' ),
				'desc'      => __( 'Select between standard Bootstrap\'s panel classes', 'four7' ),
				'id'        => 'single_meta_cats_panel_class',
				'default'   => 'default',
				'type'      => 'select',
				'required'    => array('single_meta_cats','=',array('1')),
				'options'   => array(
				    'none'    => 'none',
					'default'    => 'Default',
					'primary'    => 'Primary',
            ...
            $fields[] = array(
                'title'     => __( 'Show Author box on single posts', 'four7' ),
                'desc'      => __( 'Toggle Author Box in the footer of single posts. Default: OFF.', 'four7' ),
                'id'        => 'authorbox',
                'default'   => 0,
                'type'      => 'switch',
            );
            ...
            $fields[] = array( 
				'title'     => __( 'Author Box Panel Styling', 'four7' ),
				'desc'      => __( 'Select between standard Bootstrap\'s panel classes', 'four7' ),
				'id'        => 'authorbox_panel_class',
				'default'   => 'default',
				'type'      => 'select',
				'required'    => array('authorbox','=',array('1')),
				'options'   => array(
				    'none'    => 'none',
					'default'    => 'Default',
					'primary'    => 'Primary',
            ...
            $fields[] = array(
                'id'          => 'four7_entry_author_config',
                'title'       => __( 'Activate and order Author Meta elements', 'four7' ),
                'options'     => array(
                    'twitter'    => 'Twitter',
                    'facebook'    => 'Facebook',
                    'google_plus'=> 'Google+',
        ...
        function meta_custom_render() {
            ...
            switch ($format)
            {
                ...
                break;
                case "Chat":
                $format_icon = 'comments-o';
                break;
                case "Aside":
                $format_icon = 'dot-circle-o';
                break;
                case "Gallery":
                $format_icon = 'th';
                break;
            ...
            if ( ! $format == '' )
            $content .= sprintf( $fs_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="fa fa-'.$format_icon.' icon"></i> <a class="entry-format" href="%1$s" title="%2$s">%3$s</a>' . $fs_framework->close_col( 'span' ),
            ...
            if ( is_array( $metas ) ) {
                foreach ( $metas as $meta => $value ) {
                    ...
                    if ( $meta == 'sticky' && ! empty( $value ) && is_sticky() ) {
                        $content .= $fs_framework->open_col( 'span', array( 'medium' => $col ), null, 'featured-post' ) . '<i class="fa fa-flag icon"></i> ' . __( 'Sticky', 'four7' ) . $fs_framework->close_col( 'span' );
                    }
                    ...
                    if ( $meta == 'category' && ! empty( $value ) ) {
                        if ( $categories_list ) {
                            $content .= $fs_framework->open_col( 'span', array( 'medium' => $col ), null, 'categories-links' ) . '<i class="fa fa-folder-open-o icon"></i> ' . $categories_list . $fs_framework->close_col( 'span' );
                        }
                    }
                    ...
                    if ( $meta == 'tags' && ! empty( $value ) ) {
                        if ( $tag_list ) {
                            $content .= $fs_framework->open_col( 'span', array( 'medium' => $col ), null, 'tags-links' ) . '<i class="fa fa-tags icon"></i> ' . $tag_list . $fs_framework->close_col( 'span' );
                        }
                    }

                    // output author element
                    if ( $meta == 'author' && ! empty( $value ) ) {
                        $content .= sprintf( $fs_framework->open_col( 'span', array( 'medium' => $col ), null, 'author vcard' ) . '<i class="fa fa-user icon"></i> <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a>' . $fs_framework->close_col( 'span' ),
