...
function four7_head_cleanup() {
	...
	if ( ! class_exists( 'WPSEO_Frontend' ) ) {
		...
		add_action( 'wp_head', 'four7_rel_canonical' );
	}
}
...
function four7_language_attributes() {
	...
	$output = apply_filters( 'four7_language_attributes', $output );
 ...
 */
function four7_body_class( $classes ) {
	// Add post/page slug
 ...
 */
function four7_remove_dashboard_widgets() {
 ...
 */
function four7_caption( $output, $attr, $content ) {
	if ( is_feed() ) {
		return $output;
	}
 ...
 */
function four7_request_filter( $query_vars ) {
...
function four7_paginate_links( $args = '' ) {
	...
	$defaults = array(
		...
		'show_all'     => false,
		'prev_next'    => true,
		'prev_text'    => __( '&laquo; Previous', 'four7' ),
		'next_text'    => __( 'Next &raquo;', 'four7' ),
		'end_size'     => 1,
		'mid_size'     => 2,
		'type'         => 'plain',
	...
	switch ( $type ) :
		case 'array' :
			...
			break;
		case 'list' :
			$r .= '<ul class="page-numbers ' . $fs_framework->pagination_ul_class() . '">';
			$r .= join( '', $page_links );
			$r .= '</ul>';
			break;
		default :
 ...
 */
function four7_pagination_toggler() {
	global $wp_query;
