 ...
 * @subpackage Template
 */
class four7_Widget_Thumbnails extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'inevisys-thumbnails-widget',
			'description' => esc_html__( 'Display post images from selected category.', 'inevisys' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'inevisys-thumbnails-widget', // $this->id_base
			__( 'LMC: Thumbnails', 'inevisys' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		<!-- thumbnails list -->
		<?php if ( $maxitems == 0 ) { ?>
			<li><?php esc_html_e( 'No images.', 'inevisys' ); ?></li>
		<?php
		} else {
				...
				<ul class="loop-thumbnails">
					...
					<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
						...
						<?php if ( current_theme_supports( 'get-the-image' ) ) { ?>
							...
							<li class="<?php echo 'number-' . $thumbcounter;
							echo ( $thumbcounter % 2 == 0 ) ? ' even' : ' odd'; ?>">
	...
	function form( $instance ) {
		...
		/* Default form values. */
		$defaults = array(
			'title'                => esc_attr__( 'Thumbnails', 'inevisys' ),
			'thumbnails_cat'       => 1,
			'thumbnails_count'     => 4,
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'inevisys' ); ?>:</label><br />
		...
		<p>
			...
			<select class="widefat" id="<?php echo $this->get_field_id( 'thumbnails_cat' ); ?>" name="<?php echo $this->get_field_name( 'thumbnails_cat' ); ?>">
				<option value="1"><?php echo esc_attr( __( 'Select Category', 'inevisys' ) ); ?></option>
				<?php
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumbnails_count' ); ?>"><?php _e( 'Number of thumbnails', 'inevisys' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'thumbnails_randomize' ); ?>">
				<input class="checkbox" type="checkbox" <?php checked( $instance['thumbnails_randomize'], true ); ?> id="<?php echo $this->get_field_id( 'thumbnails_randomize' ); ?>" name="<?php echo $this->get_field_name( 'thumbnails_randomize' ); ?>" /> <?php _e( 'Display in random order', 'inevisys' ); ?>
			</label>
		</p>
