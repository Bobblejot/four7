...
class four7_Widget_flickr extends WP_Widget {

	function four7_Widget_flickr() {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'flickr_id' ); ?>"><?php _e( 'Flickr ID', 'four7' ); ?>:</label>
			...
			<small>Don't know your ID? Head on over to <a href="<?php echo $id_getter_url; ?>">idgettr</a> to find it.
			</small>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of Photos', 'four7' ); ?>:</label>
...
add_action( 'widgets_init', 'load_flickr_widget' );
