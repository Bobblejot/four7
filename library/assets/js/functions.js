...
(function(){
	...
	var page = {
		init: function () {
			...
			if (jQuery('.full-width-area').length > 0) {
				page.fullWidthArea();
				$window.smartresize( function() {
					page.fullWidthArea();
				});	
			...
			// STICKY SIDEBAR
			if (!isMobileAlt && fssIncluded.hasClass('stickysidebars') && jQuery('.sidebar').length > 0) {
				page.stickySidebars();
			}
	...
	var header = {
		init: function() {
			...
			if (fssIncluded.hasClass('sticky-header-mobile')) {
	...
	var nav = {
		init: function() {
			...
			// Set up Mega Menu
			if (!isMobile && !fssIncluded.hasClass('disable-megamenu')) {
				nav.megaMenu();
			}
	...
	var widgets = {
		init: function() {
			...
			// CHARTS
			if (fssIncluded.hasClass('has-chart')) {
			...
			if (fssIncluded.hasClass('has-countdown')) {
			widgets.countdownAssets();
			}
			
			if (fssIncluded.hasClass('has-imagebanner')) {
	...
	var $window = jQuery(window),
		body = jQuery('body'),
		fssIncluded = jQuery('#sf-included'),
	...
	var onReady = {
		init: function(){
			...
			nav.init();
			if (fssIncluded.hasClass('has-products') || body.hasClass('woocommerce-cart') || body.hasClass('woocommerce-account')) {
			woocommerce.init();
			}
			if (fssIncluded.hasClass('has-portfolio')) {
			portfolio.init();
			}
			if (fssIncluded.hasClass('has-portfolio-showcase')) {
			portfolio.portfolioShowcaseInit();
			}
			if (fssIncluded.hasClass('has-blog')) {
			blog.init();
			}
			if (fssIncluded.hasClass('has-infscroll') && !isMobile) {
			blog.infiniteScroll();
			}
			widgets.init();
			if (fssIncluded.hasClass('has-team')) {
			teamMembers.init();
			}
			if (fssIncluded.hasClass('has-carousel')) {
			carouselWidgets.init();
			woocommerce.productCarousel();
			}
			if (fssIncluded.hasClass('has-parallax')) {
			parallax.init();
			}
	...
	var onLoad = {
		init: function(){
			flexSlider.init();
			if (fssIncluded.hasClass('has-gallery')) {
			flexSlider.gallery();
			}
			if (fssIncluded.hasClass('has-chart')) {
			widgets.charts();
			}
			if (fssIncluded.hasClass('has-progress-bar')) {
			widgets.initSkillBars();
			}
			if (fssIncluded.hasClass('has-map')) {
			map.init();
			}
