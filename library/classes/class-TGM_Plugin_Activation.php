...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function install_plugins_page() {
			...
			if ( isset( $_POST[sanitize_key( 'action' )] ) && 'tgmpa-bulk-install' == $_POST[sanitize_key( 'action' )] && $plugin_table->procefs_bulk_actions() || $this->do_plugin_install() ) {
				return;
			}
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismifs_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', $this->domain ) . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		 ...
		 * @since 3.3.0
		 */
		public function procefs_bulk_actions() {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function run( $options ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					$this->skin->feedback( 'procefs_failed' );
				} /** The plugin install is successful */
				else {
					$this->skin->feedback( 'procefs_success' );
				}
			...
			public function install_strings() {
				...
				$this->strings['procefs_failed']      = __( 'Plugin install failed.', TGM_Plugin_Activation::$instance->domain );
				$this->strings['procefs_success']     = __( 'Plugin installed successfully.', TGM_Plugin_Activation::$instance->domain );
