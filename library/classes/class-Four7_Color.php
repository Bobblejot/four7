...
if ( ! class_exists( 'Four7_Color' ) ) {
	...
	class Four7_Color {
		...
		public static function brightest_dull_color( $colors = array(), $context = 'key' ) {
			...
			foreach ( $colors as $color ) {
				...
				if ( $brightest_dull ) {
					...
					$dullnefs_old = 1 / $hsv_old['s'];
				}

				if ( ! $brightest_dull || self::get_brightness( $hex ) * $dullness > self::get_brightness( $brightest_dull ) * $dullnefs_old ) {
					$brightest_dull = $hex;
				}
			}
		 ...
		 */
		public static function brightnefs_difference( $color_1 = '#ffffff', $color_2 = '#000000' ) {
			$color_1 = self::sanitize_hex( $color_1 );
			$color_2 = self::sanitize_hex( $color_2 );
