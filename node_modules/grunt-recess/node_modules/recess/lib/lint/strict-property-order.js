...
var _ = require('underscore')
	, util = require('../util')
	, RULE = {
		type: 'strictPropertyOrder', message: 'Incorrect property order for rule'
	}
...
module.exports = function (def, data) {
	...
	sortedRules = _.sortBy(cleanRules, function (rule) {
		...
		if (!~val) {
			return util.throwError(def, {
				type: 'propertyNotFound', message: 'Unknown property name: "' + rule.name + '"'
			})
		}
	...
	util.throwError(def, {
		type: RULE.type, message: RULE.message + selector + '\n\n  Correct order below:\n'.grey, extract: extract, sortedRules: sortedRules, line: firstLine
	})
